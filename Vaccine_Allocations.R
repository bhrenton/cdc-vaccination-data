library(tidyverse)
pfizer <- read_csv("https://data.cdc.gov/resource/saz5-9hgg.csv") %>% 
...
moderna <- read_csv("https://data.cdc.gov/resource/b7pe-5nws.csv") %>% 
  mutate(key= paste(jurisdiction, week_of_allocations))%>% 
  rename(moderna_1st_dose = `_1st_dose_allocations`) %>% 
  rename(moderna_2nd_dose = `_2nd_dose_allocations`)
janssen <- read_csv("https://data.cdc.gov/resource/w9zu-fywh.csv") %>% 
  mutate(key= paste(jurisdiction, week_of_allocations)) %>% 
